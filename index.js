...
function show(hash){
  ...
  // Footer: show only on home
  const footer = document.getElementById("footer");
  if(footer){
    if(target === "home") footer.removeAttribute("hidden");
...
function renderClubs(){
  ...
  const reversed = clubs.slice().reverse();

  cont.innerHTML = reversed.map(c=> `
    <div class="card club-card spacious">
      <img class="club-logo" src="${c.imageDataUrl || '/placeholder.svg?height=64&width=64'}" alt="${escapeHtml(c.name)} logo" />
      <div style="flex:1">
...
function renderAttendees(){
  ...
  if(ev){
    const badges = managerHeader.querySelectorAll(".badge");
    if(badges[2]) badges[2].textContent = `${(ev.attendees||[]).length} checked-in`;
  }
}
...
// Tabs click should just update hash (default anchor behavior is fine)
document.querySelectorAll('.tab').forEach(a=>{
  a.addEventListener("click", ()=>{ /* default anchor behavior is fine */ });
});

// Compression: canvas-based (used by both events and clubs)
...
let clubImageDataUrl = "";

if(clubLogoInput){
  clubLogoInput.addEventListener("change", async (e)=>{
    const file = e.target.files?.[0];
    clubImageDataUrl = "";
    clubLogoPreview.textContent = "";
    if(!file) return;

    try{
      // compress to reasonable size for logos
      clubImageDataUrl = await compressImage(file, 600, 0.8);
      const kb = Math.round((clubImageDataUrl.length * 3/4) / 1024);
      clubLogoPreview.innerHTML = `<div>Selected logo (${kb} KB)</div><img alt="Logo preview" style="margin-top:8px; max-height:120px; border-radius:8px;" src="${clubImageDataUrl}" />`;
    }catch(err){
      clubLogoPreview.textContent = "Could not load logo.";
    }
  });
}
...
if(clubForm){
  clubForm.addEventListener("submit", (e)=>{
    ...
    const clubs = readClubs();
    clubs.push({ id: uid(), name, description: desc, imageDataUrl: clubImageDataUrl || "" });
    writeClubs(clubs);
